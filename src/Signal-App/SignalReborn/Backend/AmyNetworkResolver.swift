...
final class AmyNetworkResolver {
    ...
    public func image(_ url: String, method: String = "GET", headers: [String: String] = [:], cache: Bool = true, scale: CGFloat? = nil, _ completion: @escaping ((_ refresh: Bool, _ image: UIImage?) -> Void)) -> UIImage? {
    ...
    public func image(_ url: URL, method: String = "GET", headers: [String: String] = [:], cache: Bool = true, scale: CGFloat? = nil, _ completion: @escaping ((_ refresh: Bool, _ image: UIImage?) -> Void)) -> UIImage? {
        var pastData: Data?
    ...
    public func imageCache(_ url: URL, scale: CGFloat? = nil) -> (Bool, UIImage?) {
        let encoded = url.absoluteString.toBase64
        let path = cacheDirectory.appendingPathComponent("\(encoded).png")
        if let data = try? Data(contentsOf: path) {
            if let image = (scale != nil) ? UIImage(data: data, scale: scale!) : UIImage(data: data) {
